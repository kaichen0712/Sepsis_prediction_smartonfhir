...
本系統由**臺北榮民總醫院**開發,是一個基於 **SMART on FHIR** 國際標準的整合型臨床決策支援平台,旨在協助醫護人員提升臨床照護品質與工作效率。系統整合了多項先進技術,包含 AI 敗血症風險預測、智慧醫療筆記生成、完整臨床資料檢視與彈性資料選擇等功能,提供一站式醫療資訊管理解決方案。

### 核心功能亮點

**AI 敗血症風險預測**: 敗血症可能導致器官功能衰竭與危及生命,本系統透過深度學習模型分析病患生理監測數值(心率、呼吸率、體溫、血氧、血壓),數秒內完成風險評估,無需等待血液培養結果,為黃金搶救時刻爭取寶貴時間。

**智慧醫療筆記**: 整合 OpenAI Whisper 語音辨識與 GPT 智能摘要技術,將醫護人員的口述內容自動轉換為結構化醫療筆記,大幅節省文書作業時間,讓醫護人員能專注於病患照護。

**完整臨床資料整合**: 透過 FHIR 標準 API 整合電子病歷系統,統一呈現病患基本資料、生命徵象、診斷紀錄、檢驗報告、用藥資訊與過敏史等完整臨床資訊,提供醫護人員全方位的病患資料視角。

**彈性資料選擇**: 提供資料篩選工具,支援依資料類型、時間範圍等條件快速定位所需資訊,方便進行臨床分析與決策參考。

本專案採用 **Next.js 15**、**React 19**、**TypeScript 5** 等現代化技術棧,搭配 **shadcn/ui (Radix)** 元件庫與 **fhirclient** 函式庫,可與任何符合 FHIR R4 標準的電子病歷系統無縫整合,提供安全、高效、易用的使用者體驗。

---

## 隱私權政策說明 (Privacy Policy)

本系統於設計階段即將隱私保護納入核心考量,透過多層次的安全機制確保病患資料安全:

### 敗血症預測模型服務

- **最小化資料傳輸**: 預測模型僅接收必要的生理訊號數據(心率、呼吸率、體溫、血氧、血壓)、年齡與性別,不傳送姓名、身分證字號、聯絡方式等可直接識別個人身分之資訊
- **技術性識別碼**: 傳送之 ID 為 FHIR 系統內部技術識別碼結合日期時間戳記,非個人身分識別資訊,無法單獨用於識別特定個人
- **低再識別風險**: 僅憑年齡、性別與生理數值等有限資訊,實務上難以反向識別特定個人身分
- **單純訊號處理**: 模型服務專注於生理訊號分析與風險預測,不儲存、不處理任何病患個資

### 應用程式安全機制

- **SMART on FHIR 標準授權**: 採用國際標準 OAuth 2.0 PKCE 授權機制,確保嚴謹的身分驗證流程
- **病患資料存取控管**: 僅授權使用者可存取對應病患之臨床資料,實施最小權限原則
- **HTTPS 全程加密**: 所有資料傳輸皆透過 HTTPS 加密協定,防止資料於傳輸過程中遭攔截
- **GitHub 託管服務**: 應用程式部署於 GitHub Pages (Microsoft),採用全球領先之企業級資安控管機制,確保平台層級的安全性

### 系統日誌與資料使用聲明

- **Log記錄規劃**: 本系統預計記錄使用者登入時間、存取紀錄等系統日誌,作為資安稽核與系統用量統計之用途
- **用途限制**: Log資料僅供資安稽核與用量計算使用,不做任何商業用途
- **第三方政策**: Log資料不提供給任何第三方機構或組織

透過上述多重防護措施,本系統將隱私洩露風險降至最低,兼顧臨床應用便利性與病患資料保護。

---

## 目錄 (Table of Contents)

1. [隱私權政策說明](#隱私權政策說明-privacy-policy)
2. [功能概述](#功能概述-feature-overview)
   - [核心功能](#核心功能)
   - [業務價值](#業務價值)
   - [技術特點](#技術特點)
3. [功能模組詳細說明](#功能模組詳細說明-feature-modules)
4. [技術架構](#技術架構-technical-architecture)
5. [API 串接說明](#api-串接說明-api-integration)
6. [系統需求](#系統需求-prerequisites)
7. [安裝與執行](#安裝與執行-install--run)
8. [SMART Sandbox 整合設定](#smart-sandbox-整合設定-smart-sandbox-integration)
9. [線上展示](#線上展示-online-demo)
10. [專案結構](#專案結構-project-structure)
11. [開發團隊](#開發團隊-development-team)
12. [致謝](#致謝-credits)
13. [授權](#授權-license)
14. [聯絡資訊](#聯絡資訊-contact)
